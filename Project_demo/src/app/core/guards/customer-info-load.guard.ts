...
import {SelectedCustomerGuard} from 'app/core/store/select-customer/select-customer-selected.resolve';
...
export class CustomerInfoLoadGuard extends BaseGuard<StateDate> implements DependencyTreeCanActivate {
  ...
  versionChecker(t: StateDate, params: { [p: string]: string }): boolean {
    return t.customerInfo.customerId === (t.activeCustomer ? t.activeCustomer.id : '');
  }
  ...
  getDependencies() {
    return [GetDefinitionsGuard, SelectedCustomerGuard];
  }
